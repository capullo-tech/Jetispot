...
class SpApiExecutor @Inject constructor(
    ...
    suspend inline fun <reified T> getJson(
        edge: Edge = Edge.Internal,
        suffix: String,
        params: Map<String, String>
    ) = get<T>(edge, suffix, params) { res ->
        res.body!!.source().use {
    ...
    suspend fun <T> get(
        ...
        val url = "$suffix?" + (params + if (edge == Edge.Internal) arrayOf(
            ...
            "podcast" to "true",
            "application" to "nft",
        ) else arrayOf()).map { "${it.key}=${URLEncoder.encode(it.value, "UTF-8")}" }
            .joinToString("&")

        val headers = Headers.headersOf(
            "User-Agent",
            "Spotify/8.7.20.1261 Android/32 (Pixel 4a (5G))",
            "Spotify-App-Version",
            "8.7.20.1261",
            "Authorization",
            "Bearer ${sessionManager.session.tokens().get("playlist-read")}"
        )
        ...
        when (edge) {
            is Edge.Internal -> {
                ...
                sessionManager.session.client().newCall(
                    Request.Builder().method("GET", null).url("https://${edge.domain}$url")
    ...
    class StatusCodeException(response: Response) :
